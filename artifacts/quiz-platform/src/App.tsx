...
function PlayPage() {
  ...
  return <PageFrame><TopNav /><main className="mx-auto max-w-3xl px-5 py-8 sm:py-14"><div className="flex items-center justify-between text-xs font-bold text-[hsl(var(--muted-foreground))]"><span className="font-mono tracking-[.16em]">{code}</span><span data-testid="text-question-progress">QUESTION {String(current + 1).padStart(2, '0')} / {String(questions.length).padStart(2, '0')}</span></div><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-[hsl(var(--muted))]"><div className="h-full rounded-full bg-[hsl(var(--accent))] transition-all duration-500" style={{ width: `${((current + 1) / questions.length) * 100}%` }} /></div><div className="mt-12"><p className="text-sm font-semibold text-[hsl(var(--primary))]">Question {current + 1}</p><h1 className="mt-3 font-display text-4xl font-bold leading-[1.03] tracking-[-.06em] sm:text-6xl" data-testid={`text-question-${current}`}>{question.prompt}</h1><div className="mt-10 grid gap-3 sm:grid-cols-2">{question.answers.map((answer, index) => <button type="button" key={answer} onClick={() => setSelected(index)} className={cx('focus-ring surface surface-hover flex min-h-16 items-center gap-4 rounded-2xl px-4 text-left text-sm font-semibold', selected === index && 'border-[hsl(var(--primary))] bg-[hsl(var(--primary)/.1)] text-[hsl(var(--primary))]')} data-testid={`button-answer-${index}`}><span className={cx('grid size-8 shrink-0 place-items-center rounded-lg bg-[hsl(var(--muted))] font-mono text-xs', selected === index && 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]')}>{String.fromCharCode(65 + index)}</span>{answer}{selected === index && <Check className="ml-auto size-5" />}</button>)}</div><div className="mt-8 flex justify-end"><Button onClick={onAnswer} disabled={selected === null || submitAnswer.isPending} data-testid="button-submit-answer">{submitAnswer.isPending ? <Loader2 className="size-4 animate-spin" /> : null}{current === questions.length - 1 ? 'Finish quiz' : 'Lock in answer'} <ArrowRight className="size-4" /></Button></div></div></main></PageFrame>;
}
