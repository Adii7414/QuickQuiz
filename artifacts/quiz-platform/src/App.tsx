...
function LoginPage({ kind }: { kind: 'teacher' | 'moderator' }) {
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); login.mutate({ data: { email, password } }, { onSuccess: (session) => { client.setQueryData(getGetCurrentUserQueryKey(), session); setLocation(kind === 'teacher' ? '/teacher' : '/moderator'); }, onError: (error) => { if (kind === 'teacher' && isTeacherSuspended(error)) setSuspended(true); } }); };
...
function DashboardShell({ children, title, subtitle, active = 'Overview', onLogout, role = 'TEACHER' }: { children: ReactNode; title: string; subtitle: string; active?: string; onLogout: () => void; role?: 'TEACHER' | 'MODERATOR' }) {
  ...
  const nav = role === 'MODERATOR'
    ...
    : [{ label: 'Overview', href: '/teacher', icon: LayoutDashboard }, { label: 'Quizzes', href: '/teacher/quizzes', icon: BookOpen }, { label: 'Live rooms', href: '/teacher/host', icon: Radio }];
...
function QuizEditor({ quiz, onClose, onSaved }: { quiz?: Quiz; onClose: () => void; onSaved: () => void }) {
  ...
  const [timerMode, setTimerMode] = useState<'manual' | 'timer'>(quiz?.timeLimitSeconds ? 'timer' : 'manual');
  const [timeLimitSeconds, setTimeLimitSeconds] = useState(String(quiz?.timeLimitSeconds || 30));
  ...
  return <section className="mt-8 overflow-hidden rounded-2xl border border-[hsl(var(--primary)/.25)] bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))]" data-testid="panel-live-room">
    <div className="flex flex-col gap-5 p-5 sm:flex-row sm:items-start sm:justify-between sm:p-6">
      <div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.14em] text-[hsl(var(--primary-foreground)/.66)]"><span className="size-2 animate-pulse rounded-full bg-[hsl(var(--accent))]" /> {session.status === 'LOBBY' ? 'Waiting room' : session.status === 'LIVE' ? 'Live now' : session.status === 'PAUSED' ? 'Paused' : 'Results ready'}</div><h2 className="mt-2 font-display text-2xl font-bold tracking-[-.05em]">{session.quizTitle}</h2><p className="mt-1 text-sm text-[hsl(var(--primary-foreground)/.7)]"><span className="font-mono">{session.code}</span> · {session.participantCount} participants</p></div>
       <div className="flex flex-col items-stretch gap-2 sm:items-end"><div className="flex flex-wrap items-center justify-end gap-2"><button aria-label="Close live room" onClick={onClose} className="focus-ring rounded-lg p-2 text-[hsl(var(--primary-foreground)/.7)] hover:bg-[hsl(var(--primary-foreground)/.1)]"><X className="size-4" /></button>{session.status === 'LOBBY' && <Button variant="soft" onClick={onStart} disabled={starting}><Radio className="size-4" /> Start quiz</Button>}{session.status === 'LIVE' && <><Button variant="soft" onClick={() => onAction(ModerationActionInputAction.PAUSE)} disabled={actionPending}><Pause className="size-4" /> Pause</Button><Button variant="soft" onClick={onAdvance} disabled={advancing || actionPending}>{advancing ? <Loader2 className="size-4 animate-spin" /> : <SkipForward className="size-4" />} {current >= questionCount - 1 ? 'Finish quiz' : 'Next question'}</Button></>}{session.status === 'PAUSED' && <Button variant="soft" onClick={() => onAction(ModerationActionInputAction.RESUME)} disabled={actionPending}><Play className="size-4" /> Resume</Button>}{session.status !== 'COMPLETE' && <Button variant="danger" onClick={() => setConfirmation({ title: 'End this room?', message: `Ending “${session.quizTitle}” will stop the quiz for everyone and publish the current scores.`, action: ModerationActionInputAction.END })} disabled={actionPending}><X className="size-4" /> End quiz</Button>}{session.status === 'COMPLETE' && <Link href={`/teacher/results/${session.code}`} className="focus-ring inline-flex min-h-10 items-center gap-2 rounded-xl bg-[hsl(var(--primary-foreground)/.14)] px-4 text-sm font-semibold"><BarChart3 className="size-4" /> Detailed results</Link>}</div>{session.status !== 'COMPLETE' && <Button variant="soft" className="w-full border border-[hsl(var(--primary-foreground)/.25)] bg-[hsl(var(--primary-foreground)/.16)] text-[hsl(var(--primary-foreground))] hover:bg-[hsl(var(--primary-foreground)/.28)] sm:w-auto" onClick={() => onAction(session.joinFrozen ? ModerationActionInputAction.UNFREEZE_JOINS : ModerationActionInputAction.FREEZE_JOINS)} aria-pressed={session.joinFrozen} data-testid="button-teacher-freeze-session">{session.joinFrozen ? <><UserCog className="size-4" /> Allow joins</> : <><Ban className="size-4" /> Freeze joins</>}</Button>}</div>
    </div>
...
function RoleGuard({ role, children }: { role: 'TEACHER' | 'MODERATOR'; children: ReactNode }) {
  ...
  }, [role, setLocation, userQuery.data, userQuery.isError, userQuery.isFetching, suspended]);
  if (suspended) return <SuspendedScreen />;
  if (userQuery.isLoading || userQuery.isFetching || !userQuery.data || userQuery.data.role !== role) {
