  ...
  return <DashboardShell title="Teacher workspace" subtitle="Build a room worth showing up for." onLogout={() => logout.mutate(undefined, { onSuccess: () => setLocation('/teacher/login') })}>
    <div className="animate-rise">
      ...
      <div className="mt-10 flex items-center justify-between"><div><h2 className="font-display text-xl font-bold tracking-[-.04em]">Your quizzes</h2><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">Draft, edit, and send one live.</p></div><button aria-label="Refresh quizzes" onClick={() => quizQuery.refetch()} className="focus-ring rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" data-testid="button-refresh-quizzes"><RefreshCw className="size-4" /></button></div>
       <div className="mt-4">{quizQuery.isLoading ? <LoadingState label="Loading your quizzes" /> : quizQuery.isError ? <ErrorState retry={() => quizQuery.refetch()} /> : quizzes.length === 0 ? <EmptyQuizzes onCreate={() => setEditor('new')} /> : <div className="grid gap-3">{quizzes.map((quiz) => <QuizRow key={quiz.id} quiz={quiz} onEdit={() => setEditor(quiz)} onDelete={() => { if (window.confirm('Delete this quiz?')) deleteQuiz.mutate({ id: quiz.id }, { onSuccess: () => client.invalidateQueries({ queryKey: getListQuizzesQueryKey() }) }); }} onHost={() => setLocation(`/teacher/host?quiz=${encodeURIComponent(quiz.id)}`)} deleting={deleteQuiz.isPending} />)}</div>}</div>
    </div>
...
function HostPage() {
  ...
  return <DashboardShell title="Live room host" subtitle="Set the pace. Watch the room come alive." active="Live rooms" onLogout={() => logout.mutate(undefined, { onSuccess: () => setLocation('/teacher/login') })}>
    <div className="animate-rise">
      ...
      {roomCode && roomQuery.data ? <LiveRoomPanelV2 session={roomQuery.data} onStart={() => start.mutate({ code: roomCode }, { onSuccess: updateRoom })} onAdvance={() => advance.mutate({ code: roomCode }, { onSuccess: updateRoom })} starting={start.isPending} advancing={advance.isPending} onClose={() => setRoomCode('')} /> : <div className="mt-9 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
    ...
    {rooms.length === 0 ? <div className="surface rounded-2xl p-8 text-center"><Radio className="mx-auto size-8 text-[hsl(var(--muted-foreground))]" /><p className="mt-3 font-semibold">No active rooms right now.</p><p className="mt-1 text-sm text-[hsl(var(--muted-foreground))]">Rooms appear here when a teacher opens one.</p></div> : <div className="grid gap-3">{rooms.map((room) => {
      ...
      return <div key={room.code} className="surface overflow-hidden rounded-2xl" data-testid={`row-moderator-session-${room.code}`}>
        <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-start sm:justify-between sm:p-5">
          ...
          <div className="flex flex-wrap gap-2 sm:justify-end"><span className="inline-flex min-h-9 items-center rounded-lg bg-[hsl(var(--muted)/.65)] px-3 text-xs font-bold">{room.participantCount} participants</span>{room.status === 'LIVE' && <Button variant="soft" className="min-h-9 px-3 text-xs" onClick={() => onAction(room.code, ModerationActionInputAction.PAUSE)} disabled={pending} data-testid={`button-pause-session-${room.code}`}><Pause className="size-3.5" /> Pause</Button>}{room.status === 'PAUSED' && <Button className="min-h-9 px-3 text-xs" onClick={() => onAction(room.code, ModerationActionInputAction.RESUME)} disabled={pending} data-testid={`button-resume-session-${room.code}`}><Play className="size-3.5" /> Resume</Button>}{room.status !== 'COMPLETE' && <Button variant="danger" className="min-h-9 px-3 text-xs" onClick={() => { if (window.confirm(`End ${room.quizTitle}?`)) onAction(room.code, ModerationActionInputAction.END); }} disabled={pending} data-testid={`button-end-session-${room.code}`}><X className="size-3.5" /> End</Button>}<Button variant="ghost" className="min-h-9 px-3 text-xs" onClick={() => onAction(room.code, room.joinFrozen ? ModerationActionInputAction.UNFREEZE_JOINS : ModerationActionInputAction.FREEZE_JOINS)} disabled={pending} data-testid={`button-freeze-session-${room.code}`}>{room.joinFrozen ? <><UserCog className="size-3.5" /> Allow joins</> : <><Ban className="size-3.5" /> Freeze joins</>}</Button><button className="focus-ring rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))]" onClick={() => setExpanded(isExpanded ? null : room.code)} aria-expanded={isExpanded} data-testid={`button-expand-session-${room.code}`}><ChevronDown className={cx('size-4 transition-transform', isExpanded && 'rotate-180')} /></button></div>
        </div>
        {isExpanded && <div className="border-t border-[hsl(var(--border))] bg-[hsl(var(--muted)/.28)] p-4 sm:p-5"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[.12em] text-[hsl(var(--muted-foreground))]">Participants</p><span className="text-xs font-semibold text-[hsl(var(--muted-foreground))]">Question {room.questionCount ? `${Math.min(room.currentQuestion + 1, room.questionCount)} / ${room.questionCount}` : '—'}</span></div>{room.participants.length === 0 ? <p className="rounded-xl bg-[hsl(var(--card))] p-4 text-sm text-[hsl(var(--muted-foreground))]">No participants have joined.</p> : <div className="grid gap-2 sm:grid-cols-2">{room.participants.map((participant) => <div key={participant.id} className="flex min-w-0 items-center gap-3 rounded-xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-3"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-[hsl(var(--secondary))] text-xs font-bold text-[hsl(var(--primary))]">{participant.name.slice(0, 1).toUpperCase()}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold">{participant.name}</span><span className="block text-xs text-[hsl(var(--muted-foreground))]">{participant.answered} answered · {participant.percentage}%</span></span><button className="focus-ring rounded-lg p-2 text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--destructive)/.1)] hover:text-[hsl(var(--destructive))]" onClick={() => { if (window.confirm(`Remove ${participant.name}?`)) onAction(room.code, ModerationActionInputAction.REMOVE_PARTICIPANT, participant.id); }} disabled={pending} title="Remove participant" data-testid={`button-remove-participant-${participant.id}`}><UserX className="size-4" /></button></div>)}</div>}</div>}
      </div>;
    })}</div>}
  </section>;
}
